...
dialog_box(int arg_color, char argc, ...)
{
  ...
  //  mouse_handle = mouse_register(dialog_window,dialbox_mouse_handler)
  refresh_dialog_box();
}
...
void refresh_dialog_box(void)
{
  ...
  int lx, ly;  /* Line positioning */
  int bx, by;  /* Button positioning */
  ...
  for (li = 0; li < ln; li++)
    {
      lx = ((wt - lw[li]) / 2);
      ly = (li * (CHAR_HEIGHT + DB_V_SPACE));
      Fgl_write(lx + text_window.x, ly + text_window.y, l[li]->text);
    }
  ...
  for (bi = 0; bi < bn; bi++)
	...
	bx = (bx + bw[bi-1] + bs);
      ...
      by = (ln * (CHAR_HEIGHT + DB_V_SPACE) + DB_V_SPACE);
      Fgl_fillbox(bx + text_window.x - BUTTON_BORDER,
		  by + text_window.y - BUTTON_BORDER,
		  bw[bi] + (BUTTON_BORDER * 2),
		  CHAR_HEIGHT + (BUTTON_BORDER * 2),
		  white(0));
      Fgl_write(bx + text_window.x, by + text_window.y, b[bi]->text);
    }
